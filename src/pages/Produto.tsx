...
import ShippingCalculator, {
  type ShippingOption,
} from "@/components/shipping/ShippingCalculator";
...
const Produto = () => {
  ...
  const [finishingQuantities, setFinishingQuantities] = useState<
    ...
    const load = async () => {
      ...
      if (data?.id) {
        ...
        if (pfLinks && pfLinks.length > 0) {
          ...
          console.log(
            "Nenhum link de acabamento encontrado para este ID de produto:",
            data.id,
          );
  ...
  useEffect(() => {
    ...
    if (qtyVal) {
      ...
      Object.entries(configValues).forEach(([key, val]) => {
        if (
          typeof val === "string" &&
          (val.includes(" UN") || val.includes("unidade"))
        ) {
  ...
  const sqmPresets = useMemo(() => {
    ...
    const item = product.configuration_schema.find(
      (it: any) => it.ui_type === "sqm_presets",
    );
    return item?.options || [];
  ...
  const calculatedPrice = useMemo(() => {
      ...
      product.configuration_schema.forEach((item: any) => {
        ...
        if (item.ui_type === "hierarchy" && Array.isArray(item.groups)) {
          ...
          const group = item.groups.find(
            (g: any) => g.id === groupId || g.name === groupId,
          );
          if (group) {
            const opt = Array.isArray(group.options)
              ? group.options.find((o: any) => o.name === val)
              : null;
            if (opt) {
          ...
          val.forEach((v: string) => {
            const opt = Array.isArray(item.options)
              ? item.options.find((o: any) => o.name === v)
              : null;
            if (opt) otherAdjs += Number(opt.price_adj) || 0;
          });
        } else {
          const opt = Array.isArray(item.options)
            ? item.options.find((o: any) => o.name === val)
            : null;
          if (opt) otherAdjs += Number(opt.price_adj) || 0;
        }
      });
    ...
    if (hasHierarchySelected) {
      ...
      // Para produtos por m², calcula: área × preço/m² × quantidade de itens
      baseTotal = area * pricePerSqm * selectedQuantity;
    } else {
      // Para produtos com preço fixo por pacote (ex: 1000 cartões = R$100),
      // o baseUnitPrice já é o preço total do pacote — não multiplicar pela quantidade.
  ...
  const handleAddToCart = () => {
    ...
    const configDetails = product.configuration_schema
      ?.map((item: any) => {
        const val = configValues[item.id];
        if (!val) return null;
    ...
    addItem({
      ...
      quantity: 1,
      productionQuantity: selectedQuantity,
  ...
  if (loading)
    return (
      <PublicLayout>
        <div className="py-20 text-center animate-pulse">Carregando...</div>
      </PublicLayout>
    );
  if (!product)
    return (
      <PublicLayout>
        <div className="py-20 text-center">Produto não encontrado</div>
      </PublicLayout>
    );
    ...
    <PublicLayout>
      ...
      <article className="py-12 container mx-auto px-4 max-w-6xl mt-6 lg:mt-10">
        <nav className="flex items-center gap-1.5 text-sm mb-10 flex-wrap">
          <Link
            to="/loja"
            className="text-muted-foreground hover:text-highlight transition-colors flex items-center gap-1"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-none">
                {images.map((img: any, i: number) => (
                  <button
                    ...
                    className={`w-20 h-20 rounded-2xl overflow-hidden border-2 transition-all ${i === selectedImage && !selectedImageOverride ? "border-primary" : "border-transparent opacity-50"}`}
                  >
                    <img
                      src={img.image_url}
          ...
          >
              ...
              {isSqm ? (
                <div className="space-y-6">
                  <div className="flex justify-between items-center">
                    ...
                    >
                      <Ruler className="w-3 h-3 mr-1" /> M²
                    </Badge>
                  </div>
                  ...
                  {sqmPresets.length > 0 && (
                    <div className="space-y-2">
                      ...
                      >
                        <option value="custom">
                          Medida Personalizada (Sob Medida)
                        </option>
                        {sqmPresets.map((p: any) => (
                          <option key={p.name} value={p.name}>
                            {p.name} ({p.width}x{p.height}m)
                          </option>
                        ))}
                      </select>
                    </div>
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground font-bold">
                    {selectedQuantity.toLocaleString()} unidades
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-8 mb-10">
              {Array.isArray(product.configuration_schema) &&
                product.configuration_schema.map(
                  (item: any, itemIdx: number) => {
                    ...
                    if (
                      ...
                      const selectedGroup = item.groups.find(
                        (g: any) =>
                          g.id === selectedGroupId ||
                          g.name === selectedGroupId,
                      );
                                ...
                                <SelectContent className="rounded-xl border-border bg-popover shadow-xl">
                                  {item.groups.map((group: any) => (
                                    <SelectItem
                                      ...
                                      className="font-medium focus:bg-primary/5 focus:text-primary py-3"
                                    >
                                      {group.name}
                                    </SelectItem>
                                  ))}
                          ...
                          {selectedGroup && (
                            ...
                            >
                              <Label className="text-sm font-black uppercase tracking-widest text-foreground/70 flex items-center gap-2">
                                ...
                                Opções Disponíveis para {selectedGroup.name}
                              </Label>
                              <div className="space-y-3">
                                <Select
                                    ...
                                    {selectedGroup.options.map(
                                      (opt: any, optIdx: number) => (
                                        <SelectItem
                                          key={optIdx}
                                          value={opt.name}
                                          className="py-3 focus:bg-highlight/5 focus:text-highlight"
                                        >
                                          <div className="flex items-center justify-between w-full gap-8 min-w-[200px]">
                                            <span className="font-bold">
                                              {opt.name}
                                            </span>
                                            <span className="font-black text-highlight">
                    ...
                    const options = Array.isArray(item.options)
                      ...
                      : [];
                    const combinations = (
                      product.configuration_schema as any[]
                    ).find((it) => it.combinations)?.combinations;
                    ...
                    if (combinations) {
                      const currentQty =
                        configValues["qty"] ||
                        product.configuration_schema.find(
                          (it: any) => it.id === "qty",
                        )?.options[0]?._val;
                      const currentCore =
                        configValues["cores"] ||
                      ...
                      if (item.id === "qty" && currentCore) {
                        filteredOptions = options.filter((opt) =>
                          combinations.some(
                            (combo: any) =>
                              combo.q === opt._val && combo.c === currentCore,
                          ),
                        );
                        ...
                        filteredOptions = options.filter((opt) =>
                          combinations.some(
                            (combo: any) =>
                              combo.q === currentQty && combo.c === opt._val,
                          ),
                        );
                      }
                    }
                        ...
                        {item.ui_type === "checkbox" ? (
                          <div className="bg-secondary/20 rounded-2xl p-4 space-y-3 border border-border/50">
                            {filteredOptions.map((opt: any) => {
                              ...
                              const current =
                                (configValues[item.id] as string[]) || [];
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center gap-2">
                                    <div
                                      className={`w-4 h-4 rounded border-2 transition-all flex items-center justify-center ${isSel ? "bg-primary border-primary" : "border-muted-foreground/30 group-hover:border-primary/50"}`}
                                    >
                                      {isSel && (
                                        <Check
                                    ...
                                    <span
                                      className={`text-[12px] transition-colors ${isSel ? "font-bold text-foreground" : "text-muted-foreground group-hover:text-foreground"}`}
                                    >
                                      {opt.name}
                                    </span>
                                  </div>
                                ...
                                {filteredOptions.map((opt: any) => {
                                  ...
                                  return (
                                    <SelectItem
                                      key={opt.name}
                                      value={opt.name}
                                      className="py-3 focus:bg-primary/5 focus:text-primary"
                                    >
                                      {opt.name}
                                      {costText}
                                    </SelectItem>
            ...
            {availableFinishings.length > 0 && (
              <div className="space-y-6 mb-12 bg-primary/5 p-6 rounded-3xl border border-primary/10">
                ...
                <Label className="text-lg font-black uppercase tracking-widest text-foreground flex items-center gap-3">
                  <Palette className="w-5 h-5 text-primary" /> Personalize seu
                  Acabamento
                </Label>
                <p className="text-xs text-muted-foreground -mt-2">
                  Escolha as opções extras para valorizar seu produto.
                </p>
                            ...
                            <div key={f.id} className="space-y-3">
                                    ...
                                    if (hasGroup) {
                                      ...
                                      if (foundKeyword) {
                                        ...
                                        setSelectedFinishings((p) => [
                                          ...p.filter((id) => {
                                            const other =
                                              availableFinishings.find(
                                                (af) => af.id === id,
                                              );
                                            return !other?.name
                                              .toLowerCase()
                                              .includes(foundKeyword);
                                          }),
                                          f.id,
                                        ]);
                                      } else {
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  <div
                                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? "bg-primary border-primary" : "border-muted-foreground/30 group-hover:border-primary/50"}`}
                                  >
                                    {isSelected &&
                                      (isUnit ? (
                                        <Check
                                  ...
                                  <span
                                    className={`text-[13px] transition-colors ${isSelected ? "font-bold text-foreground" : "text-muted-foreground group-hover:text-foreground"}`}
                                  >
                                    {f.name}
                                  </span>
                                </div>
                              ...
                              {isSelected && isUnit && (
                                <motion.div
                                  initial={{ opacity: 0, y: -10 }}
                                  animate={{ opacity: 1, y: 0 }}
                                  className="flex items-center justify-between px-4 py-3 bg-white rounded-xl border border-primary/20 ml-8"
                                >
            ...
            {(isSqm ||
              (!product.name?.toLowerCase().includes("cartão") &&
                !product.configuration_schema?.some(
                  (it: any) =>
                    it.label?.toLowerCase().includes("quant") ||
                    it.ui_type === "hierarchy",
                ))) && (
              <div className="space-y-4 mb-10">
                <Label className="text-sm font-black uppercase tracking-widest text-foreground/70 flex items-center gap-2">
                  <Package className="w-4 h-4 text-primary" /> Quantidade de
                  Itens
                </Label>
                  ...
                  <div className="grid grid-cols-4 gap-2.5">
                    {[25, 50, 100, 250, 500, 1000, 2500, 5000].map((qty) => (
                      ...
                      >
                        ...
                        <div
                          className={`text-[9px] uppercase font-bold tracking-tighter ${selectedQuantity === qty ? "text-white/70" : "opacity-40"}`}
                        >
                          un
                        </div>
                      </button>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              <Button
                ...
                variant="highlight"
                className="flex-1 h-14 font-black shadow-glow-sm"
              >
              ...
              <Button
                onClick={() => {
                  handleAddToCart();
                  if (canAddToCart) window.location.href = "/checkout";
                }}
                disabled={!canAddToCart}
                variant="hero"
            ...
            <div className="grid grid-cols-2 gap-3 mb-6 mt-8">
              {trustItems.map((it, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 p-2 rounded-xl bg-secondary/30 border border-border/30"
                >
            ...
            <div className="flex flex-col gap-2 mb-8 bg-success/5 p-4 rounded-2xl border border-success/10">
              <p className="text-xs text-success font-black uppercase tracking-widest flex items-center gap-2">
                <CheckCircle2 className="w-3.5 h-3.5" /> Pagamento via PIX • Sem
                taxas
              </p>
              {product.estimated_days && (
                <p className="text-xs text-muted-foreground flex items-center gap-2">
                  <Clock className="w-3.5 h-3.5 text-highlight" /> Produção
                  estimada: ~{product.estimated_days} dias úteis
                </p>
              )}
            </div>
            ...
            {product.specifications && (
              <div className="mb-8">
                ...
                <div
                  className="text-foreground/80 text-sm prose prose-sm max-w-none bg-secondary/30 rounded-3xl p-8 border border-border/50 break-words whitespace-pre-line overflow-hidden"
                  dangerouslySetInnerHTML={{
                    __html: sanitizeHTML(product.specifications),
                  }}
                />
              </div>
            )}
            ...
            <div className="bg-card border border-border rounded-3xl p-6 mb-8">
              <h3 className="font-display font-bold flex items-center gap-2 mb-4">
                <Truck className="w-5 h-5 text-primary" /> Frete e Prazos
              </h3>
              <ShippingCalculator
        ...
        <div className="mt-12 space-y-16">
          {product.full_description && (
            <section className="bg-card rounded-[32px] p-8 md:p-12 border border-border/50 shadow-sm overflow-hidden">
              <h2 className="font-display font-bold text-2xl mb-8 flex items-center gap-3">
                <Sparkles className="w-6 h-6 text-primary" /> Informações
                Complementares
              </h2>
              <div
          ...
          {(product.name?.toLowerCase().includes("cartão") ||
            product.name?.toLowerCase().includes("logo")) && (
            <ArtEditor productName={product.name} />
          )}
          <SmartRecommendations productId={product.id} />
          <RelatedProducts
            productId={product.id}
            catalogNodeId={product.catalog_node_id}
          />
        </div>
      </article>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/95 backdrop-blur-xl border-t border-border z-50 animate-in slide-in-from-bottom duration-300">
          <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
            ...
            <Button
              ...
              disabled={!canAddToCart}
              variant="highlight"
              className="flex-1 md:flex-none h-12 px-8 font-black"
            >
              CARRINHO • R$ {calculatedPrice.toFixed(2)}
            </Button>
          </div>
        </div>
      )}
