        ...
        >
          <DialogTrigger asChild>
            <Button variant="hero" onClick={() => openForm()}>
              <Plus className="w-4 h-4 mr-2" /> Novo Acabamento
            </Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {editing ? "Editar Acabamento" : "Novo Acabamento"}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="Ex: Ilhós, Batente, Corte reto"
                  required
                />
              </div>
              <div>
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="Ex: Orientação, Cor, etc."
                />
                <p className="text-[10px] text-muted-foreground mt-1">
                  Acabamentos no mesmo grupo tornam-se seleção única para o
                  cliente.
                </p>
              </div>
              <Button
            ...
            {items.map((item) => (
              ...
              >
                ...
                <td className="p-3 text-left">
                  {item.group_name ? (
                    <span className="text-[10px] font-bold bg-primary/10 text-primary px-2 py-0.5 rounded-full uppercase italic tracking-tighter">
                      {item.group_name}
                    </span>
                  ) : (
                ...
                <td className="p-3 text-left text-muted-foreground text-xs">
                  {item.pricing_mode === "per_unit" ? "Por unidade" : "Fixo"}
                </td>
                <td className="p-3 text-right text-foreground">
                  R$ {Number(item.price).toFixed(2)}
                  {item.pricing_mode === "per_unit" ? "/un" : ""}
                </td>
                <td className="p-3 text-center">
                  <button
                    onClick={() => toggleActive(item)}
                    className={`text-xs px-2 py-0.5 rounded-full cursor-pointer ${item.is_active ? "bg-success/20 text-success" : "bg-destructive/20 text-destructive"}`}
                  >
