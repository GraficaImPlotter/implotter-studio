...
const AdminProdutos = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    let minPrice = calculateCommercialRounding((unitCost + FRETE_DILUIDO) * categoryMarkup);
  ...
  return (
    <AdminLayout>
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={open} onOpenChange={v => { setOpen(v); if (!v) { setEditing(null); setSavedProductId(null); setPendingFiles([]); pendingPreviews.forEach(u => URL.revokeObjectURL(u)); setPendingPreviews([]); } }}>
          ...
          <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
            ...
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="bg-gradient-to-r from-highlight/10 to-highlight-glow/10 rounded-xl p-4 border border-highlight/20">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">Preenche descrições e SEO automaticamente.</p>
                  </div>
                  <Button type="button" variant="outline" size="sm" onClick={handleGenerateWithAI} disabled={generatingAI || !productName.trim()}>
                    {generatingAI ? <><Loader2 className="w-4 h-4 mr-1 animate-spin" /> Gerando...</> : <><Sparkles className="w-4 h-4 mr-1" /> Gerar com IA</>}
                  </Button>
                </div>
              </div>

              <div><label className="text-sm font-medium">Chamada Curta (Home)</label><Input name="short_description" defaultValue={editing?.short_description} placeholder="Ex: Cartão de visita 4x0 couche 300g" /></div>
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">Ficha Técnica</label>
                ...
                <div className="flex flex-wrap items-center gap-6 pt-6">
                 ...
                 <div className="bg-primary/5 rounded-xl p-6 border border-primary/10">
                    <h3 className="font-display font-bold text-foreground flex items-center gap-2 mb-6">
                        <Sparkles className="w-5 h-5 text-primary" /> SEO & Google AdSense 🚀
                    </h3>
                    <div className="space-y-6">
                        ...
                        <div>
                          <label className="text-sm font-bold text-foreground flex items-center gap-2 mb-2">
                             <Search className="w-4 h-4 text-highlight" /> Meta Descrição (Importante p/ AdSense)
                          </label>
                          <Textarea value={metaDesc} onChange={e => setMetaDesc(e.target.value)} className="h-20" />
                        </div>
                        <div>
                          <label className="text-sm font-bold text-foreground flex items-center gap-2 mb-2">
                             <Tag className="w-4 h-4 text-highlight" /> Palavras-Chave
                          </label>
                          <Input name="keywords" defaultValue={editing?.keywords || ""} />
                        </div>
                    </div>
                 </div>
              ...
              <div className="bg-primary/5 rounded-2xl p-6 border border-primary/20 space-y-6">
                  <div className="flex justify-between items-center mb-4">
                     <h3 className="font-display font-bold text-foreground flex items-center gap-2">
                         <Layers className="w-5 h-5 text-primary" /> Variações Hierárquicas (Preços por Lista)
                     </h3>
                     <Button type="button" variant="outline" size="sm" onClick={() => setGroupedVariants(prev => [...prev, { id: generateUUID(), name: "OPÇÃO 1", options: [{ name: "500 UN", cost: 0, price: 0 }] }])}>
                    ...
                    <div key={group.id} className="bg-background rounded-xl p-4 border border-border space-y-4 relative shadow-sm">
                       ...
                       <div className="space-y-3 ml-7">
                          ...
                          <Button type="button" variant="ghost" size="sm" className="h-8 text-[11px]" onClick={() => {
                             ...
                             setGroupedVariants(prev => prev.map(g => g.id === group.id ? { ...g, options: next } : g));
                          }}><Plus className="w-3 h-3 mr-1" /> Add Tiragem</Button>
                       </div>
                    </div>
                  ))}
              </div>
              ...
              {savedProductId ? (
                ...
                <div className="bg-secondary/50 rounded-xl p-4 border border-border space-y-3">
                    <label className="text-sm font-medium flex items-center gap-2"><ImageIcon className="w-4 h-4 text-highlight" /> Imagens do Produto</label>
                    <div className="border-2 border-dashed border-border rounded-xl p-6 text-center">
                      <p className="text-sm text-muted-foreground">Salve o produto primeiro para habilitar o upload de imagens.</p>
                    </div>
                </div>
              )}

              <Button type="submit" variant="hero" className="w-full" disabled={submitting}>
                {submitting ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Salvando...</> : editing ? "Salvar Alterações" : "Criar Produto"}
              </Button>
            </form>
          </DialogContent>
        </Dialog>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <div className="flex items-center justify-between border-b border-border pb-1">
            ...
            <label className="flex items-center gap-2 text-[10px] font-black uppercase text-muted-foreground cursor-pointer">
                <input type="checkbox" checked={groupByCategory} onChange={e => setGroupByCategory(e.target.checked)} className="rounded border-border text-primary" />
                                        ...
                                        {items.map(p => (
                                            <tr key={p.id} className="border-t border-border/30 hover:bg-muted/10 transition-colors">
                                                ...
                                                <td className="p-4 text-center">
                                                    <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${p.is_active ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
