...
const AdminOrcamentos = () => {
  ...
  const load = useCallback(async () => {
    ...
    if (searchTerm.trim()) {
      ...
      if (safe)
        q = q.or(
          `customer_name.ilike.%${safe}%,quote_number.eq.${parseInt(safe) || 0}`,
        );
    }
    const { data } = await q;
    ...
    try {
      ...
      if (data.razao_social) {
        ...
        if (data.ddd_telefone_1) {
          setFCustomerPhone(
            (prev) =>
              prev || formatPhone(data.ddd_telefone_1.replace(/\D/g, "")),
          );
        }
      }
    } catch {
  ...
  const resetForm = () => {
    ...
    setFAddressStreet("");
    setFAddressNumber("");
    setFAddressComplement("");
    setFAddressNeighborhood("");
    setFAddressCity("");
    setFAddressState("");
    setFValidUntil("");
    setFNotes("");
  ...
  const openEdit = async (quote: Quote) => {
    ...
    const { data: items } = await supabase
      ...
      .order("sort_order");
    setFormItems(
      items && items.length > 0 ? (items as QuoteItem[]) : [emptyItem()],
    );
    setOpen(true);
  };
  ...
  const selectProduct = (idx: number, product: any) => {
    ...
    setFormItems((prev) => {
      ...
      updated[idx] = {
        ...
        unit_price: price,
        subtotal: price * updated[idx].quantity,
      };
      return updated;
    });
  ...
  const updateStatus = async (quote: Quote, newStatus: string) => {
    ...
    if (newStatus === "enviado" && !quote.sent_at)
  ...
  const sendWhatsApp = async (quote: Quote) => {
    ...
    const itemsList = (items ?? [])
      .map(
        (item: any, i: number) =>
          `${i + 1}. ${item.product_name} - ${item.quantity}x R$ ${Number(item.unit_price).toFixed(2)} = R$ ${Number(item.subtotal).toFixed(2)}`,
      )
      .join("\n");
    const validUntil = quote.valid_until
      ...
      : "Não informada";
    const message =
      `*ORÇAMENTO #${quote.quote_number} - Gráfica ImPlotter*\n\nOlá ${quote.customer_name}!\n\nSegue seu orçamento:\n\n${itemsList}\n\n` +
  ...
  const handleReceipt = async (quote: Quote) => {
    ...
    generateReceiptPDF({
      ...
      items: ((items ?? []) as QuoteItem[]).map((it) => ({
        description:
  ...
  const convertToOrder = async (quote: Quote) => {
    ...
    const { data: order, error } = await supabase
      ...
      .insert({
        ...
        customer_email:
          quote.customer_email || `orcamento-${Date.now()}@implotter.local`,
    ...
    await supabase.from("order_status_history").insert({
      order_id: order.id,
      status: "pagamento_confirmado" as any,
      notes: "Orçamento aprovado - venda manual criada",
    });
    await updateStatus(quote, "aceito");
    toast({
      title: `Venda manual #${order.order_number} criada a partir do orçamento!`,
    });
  };
            ...
            <tbody>
              {quotes.map((q) => (
                ...
                >
                  ...
                  <td className="p-3">
                    <span
                      className={`text-xs px-2.5 py-1 rounded-full font-medium ${STATUS_COLORS[q.status] || ""}`}
                    >
                  ...
                  <td className="p-3">
                    <div className="flex items-center justify-end gap-1 flex-wrap">
                      <Button
                        variant="outline"
                        size="icon"
                        className="h-8 w-8 text-blue-500 border-blue-100 hover:bg-blue-50"
                        onClick={() => openView(q)}
                        title="Visualizar"
                      >
                      ...
                      <Button
                        variant="outline"
                        size="icon"
                        className="h-8 w-8 text-amber-500 border-amber-100 hover:bg-amber-50"
                        onClick={() => openEdit(q)}
                        title="Editar"
                      >
                      ...
                      <Button
                        variant="outline"
                        size="icon"
                        className="h-8 w-8 text-slate-500 border-slate-100 hover:bg-slate-50"
                        onClick={() => handlePDF(q)}
                      ...
                      <Button
                        variant="outline"
                        size="icon"
                        className="h-8 w-8 text-highlight border-highlight/20 hover:bg-highlight/10 shadow-glow-sm"
                        onClick={() => handlePremiumPDF(q)}
                      ...
                      <Button
                        variant="outline"
                        size="icon"
                        className="h-8 w-8 text-success border-success/20 hover:bg-success/10"
                        onClick={() => handleReceipt(q)}
                        title="Gerar Recibo"
                      >
                        <FileText className="w-4 h-4" />
                      </Button>
                      ...
                      {q.status !== "recusado" && (
                        <Button
                          ...
                          onClick={() => updateStatus(q, "recusado")}
                          title="Recusar"
                        >
      ...
      >
        <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editing
                ? `Editar Orçamento #${editing.quote_number}`
                : "Novo Orçamento"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFCustomerPhone(formatPhone(e.target.value))
                    }
            ...
            <div className="bg-secondary/50 rounded-xl p-4 space-y-3 border border-border">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="col-span-2 sm:col-span-3">
                  ...
                  <Input
                    value={fAddressStreet}
                    onChange={(e) => setFAddressStreet(e.target.value)}
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                  ...
                  <Input
                    value={fAddressNumber}
                    onChange={(e) => setFAddressNumber(e.target.value)}
                  />
                </div>
                <div>
                  <label className="text-sm font-medium">Complemento</label>
                  <Input
                    value={fAddressComplement}
                    onChange={(e) => setFAddressComplement(e.target.value)}
                  />
                </div>
                <div>
                  <label className="text-sm font-medium">Bairro</label>
                  <Input
                    value={fAddressNeighborhood}
                    onChange={(e) => setFAddressNeighborhood(e.target.value)}
                  />
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <div className="col-span-2">
                    ...
                    <Input
                      value={fAddressCity}
                      onChange={(e) => setFAddressCity(e.target.value)}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      value={fAddressState}
                      onChange={(e) => setFAddressState(e.target.value)}
                      maxLength={2}
                      className="uppercase"
                    />
                  </div>
            ...
            <div className="bg-secondary/50 rounded-xl p-4 border border-border space-y-3">
              ...
              <div className="flex flex-col items-end gap-2 pt-3 border-t border-border">
                ...
                <div className="flex items-center gap-3 text-lg border-t border-border pt-2">
                  ...
                  <span className="font-display font-bold text-foreground w-28 text-right">
                    R$ {formTotal.toFixed(2)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Textarea
                  rows={3}
                  value={fInternalNotes}
                  onChange={(e) => setFInternalNotes(e.target.value)}
                />
              </div>
            </div>
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-3">
                  ...
                  <span
                    className={`text-xs px-2.5 py-1 rounded-full font-medium ${STATUS_COLORS[viewing.status] || ""}`}
                  >
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-3 gap-3 text-sm">
                  ...
                  <div className="bg-secondary/50 rounded-lg p-3 border border-border">
                    ...
                    <p className="font-medium text-foreground">
                      {viewing.valid_until
                        ? formatDate(viewing.valid_until)
                        : "—"}
                    </p>
                  </div>
                  <div className="bg-secondary/50 rounded-lg p-3 border border-border">
                    ...
                    <p className="font-medium text-foreground">
                      {viewing.sent_at ? formatDate(viewing.sent_at) : "—"}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-secondary/50 rounded-xl p-4 border border-border">
                  ...
                  <div className="space-y-2">
                    {viewItems.map((item, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-start bg-background rounded-lg p-3 border border-border"
                      >
                  ...
                  <div className="mt-3 pt-3 border-t border-border space-y-1 text-right">
                    <p className="text-sm text-muted-foreground">
                      Subtotal: R$ {Number(viewing.subtotal).toFixed(2)}
                    </p>
                    {Number(viewing.discount) > 0 && (
                      <p className="text-sm text-success">
                        Desconto: -R$ {Number(viewing.discount).toFixed(2)}
                      </p>
                    )}
                    <p className="font-display text-xl font-bold text-foreground">
                      Total: R$ {Number(viewing.total).toFixed(2)}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                  ...
                  <Button
                    variant="outline"
                    onClick={() => updateStatus(viewing, "aceito")}
                    className="text-sm text-success border-success/30 hover:bg-success/10"
                    disabled={viewing.status === "aceito"}
                  >
                    <CheckCircle className="w-4 h-4 mr-1.5" /> Aceitar
                  </Button>
                  <Button
                    variant="outline"
                    onClick={() => updateStatus(viewing, "recusado")}
                    className="text-sm text-destructive border-destructive/30 hover:bg-destructive/10"
                    disabled={viewing.status === "recusado"}
                  >
                ...
                {viewing.status === "aceito" && (
                  <Button
                    variant="hero"
                    className="w-full"
                    onClick={() => convertToOrder(viewing)}
                  >
                    <ShoppingCart className="w-4 h-4 mr-2" /> Converter em
                    Pedido
                  </Button>
                )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <select
                    ...
                    onChange={(e) => updateStatus(viewing, e.target.value)}
