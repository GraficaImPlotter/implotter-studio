      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
        <div>
          ...
          <h1 className="font-display text-4xl font-black text-foreground tracking-tight">
            Cerebro <span className="text-primary">ImPlotter</span>
          </h1>
          <p className="text-muted-foreground text-sm font-medium">
            Controle central de inteligência e performance operacional
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 bg-card p-1.5 rounded-2xl border border-border shadow-sm">
          {quickStats.map((s, idx) => (
            <div
              key={idx}
              className="px-4 py-2 border-r last:border-0 border-border/50 text-center"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
        {cards.map((c, i) => (
          <motion.div
            ...
            className="glass-card-premium rounded-3xl p-6 hover:translate-y-[-4px] transition-all duration-300 border-white/5 group shadow-xl relative overflow-hidden border-glow-animated"
          >
            <div
              className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${c.color} opacity-[0.03] blur-2xl rounded-full translate-x-1/2 translate-y-[-1/2]`}
            />

            <div className="flex items-start justify-between mb-6">
              <div
                className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${c.color} flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform`}
              >
              ...
              {isLoading ? (
                ...
                <span
                  className={`px-2 py-1 rounded-lg text-[10px] font-bold flex items-center gap-0.5 ${c.up ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}
                >
                  {c.up ? (
            ...
            <div className="space-y-1">
              {isLoading ? (
                ...
                <p className="font-display text-2xl font-black text-foreground tracking-tighter">
                  {c.value}
                </p>
              )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass-card-premium rounded-3xl p-8 border-white/5 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="font-display font-black text-foreground text-2xl tracking-tight uppercase">
                Fluxo Recente
              </h2>
              <p className="text-muted-foreground text-xs font-medium tracking-wide">
                Pedidos processados nas últimas horas
              </p>
            </div>
            <a
              href="/admin/pedidos"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5 pb-4">
                  <th className="text-left py-4 font-black text-muted-foreground text-[10px] uppercase tracking-widest">
                    ID
                  </th>
                  <th className="text-left py-4 font-black text-muted-foreground text-[10px] uppercase tracking-widest">
                    Cliente
              ...
              <tbody className="divide-y divide-white/5">
                {recentOrders.map((o) => (
                  ...
                  >
                    <td className="py-5 font-black text-highlight text-xs">
                      #{o.order_number}
                    </td>
                    <td className="py-5">
        ...
        <div className="glass-card-premium rounded-3xl p-8 border-white/5 shadow-2xl flex flex-col h-full">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h3 className="font-display font-black text-foreground text-xl tracking-tight uppercase">
                Recuperar Leads
              </h3>
              <p className="text-muted-foreground text-[10px] font-medium tracking-wide">
                Carrinhos e PIX pendentes {">"} 2h
              </p>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-[8px] bg-amber-400/20 text-amber-400 px-1.5 py-0.5 rounded font-black">
                        PENDENTE
                      </span>
                    </div>
                    <a
                      ...
                      onClick={(e) => {
                        if (!hasPhone) {
                          ...
                          toast({
                            ...
                            description:
                              "Este cliente não possui um número de WhatsApp cadastrado.",
                            variant: "destructive",
                          });
                        }
                      }}
          ...
          <button
            onClick={() =>
              toast({
                ...
                description:
                  "O relatório detalhado de abandono está sendo gerado e será enviado para o seu e-mail.",
              })
            }
