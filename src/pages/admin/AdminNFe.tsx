...
const AdminNFe = () => {
  ...
  const loadConfig = async () => {
    try {
      const data = await getNFeConfig();
      if (data) setConfig(data);
    } catch (err) {
      console.log("Configuração de NF-e não encontrada", err);
    }
  };
  ...
  const loadOrders = async () => {
    try {
      const { data } = await supabase
        ...
        .or(
          "status.eq.pagamento_confirmado,status.eq.finalizado,origin.eq.manual,origin.eq.orcamento",
        )
  ...
  const handleCNPJBlur = async () => {
    ...
    try {
      ...
      if (data.inscricao_estadual)
        newConfig.inscricao_estadual = data.inscricao_estadual;
      ...
      toast({
        title: "Sucesso",
        description: "Dados da empresa carregados via CNPJ!",
      });
    } catch (error: any) {
      console.log("Erro ao buscar dados via CNPJ:", error);
    } finally {
  ...
  const handleCEPBlur = async () => {
    ...
    try {
      ...
      console.log("Erro ao buscar dados via CEP:", error);
    } finally {
      setCepLoading(false);
    }
  };
  ...
  const handleEmitNFe = async () => {
    ...
    if (!draftDestinatario || draftItens.length === 0) {
      toast({
        title: "Atenção",
        description: "Aguarde o carregamento dos dados para revisão",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "NF-e Gerada",
        description: "Chave: " + (result.accessKey?.slice(-10) || "") + "...",
      });
  ...
  const handleDeleteNFe = async (nfeId: string) => {
    if (
      !confirm(
        "Deseja realmente excluir/descartar esta Nota Fiscal local? Como ela ainda não foi enviada para a SEFAZ, você poderá gerar uma nova nota para este pedido.",
      )
    )
      return;
    ...
    try {
      ...
      toast({
        title: "Sucesso",
        description: "Nota Fiscal excluída localmente!",
      });
      loadNFe();
    } catch (error: any) {
      toast({
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: "Sucesso!",
          description:
            "NF-e assinada com sucesso! Agora você pode enviá-la para a SEFAZ.",
        });
      } else {
        toast({
  ...
  const handleSendToSEFAZ = async (nfe: NFeRecord) => {
    ...
    try {
      ...
      toast({
        title: result.success ? "Enviada" : "Erro",
        description: result.message || "NF-e enviada para SEFAZ",
  ...
  const handleCancel = async () => {
    if (!selectedNFe || cancelReason.length < 15) {
      toast({
        title: "Atenção",
        description: "Motivo deve ter pelo menos 15 caracteres",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleCertUpload = async () => {
    ...
    try {
      ...
      toast({
        title: "Sucesso",
        description: "Certificado enviado e validado com sucesso!",
      });
      if (result.warning)
        toast({
          title: "Atenção",
  ...
  const filteredList = nfeList.filter((nfe) => {
    ...
    return (
      nfe.access_key?.toLowerCase().includes(term) ||
      nfe.numero?.toString().includes(term) ||
      nfe.order_id?.toLowerCase().includes(term)
    );
  });

  const statusCounts = nfeList.reduce(
    (acc: any, nfe) => {
    ...
    <AdminLayout>
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2">
              <FileText className="h-8 w-8 text-primary" /> NF-e
            </h1>
            <p className="text-muted-foreground">
              Emissão de Nota Fiscal Eletrônica com certificado A1 (.pfx)
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          {Object.entries(NFE_STATUS_LABELS).map(
        ...
        <div className="flex items-center gap-4">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input
              placeholder="Buscar por chave, numero ou pedido..."
              value={searchTerm}
          ...
          <Button variant="outline" size="icon" onClick={loadNFe}>
                ...
                <table className="w-full min-w-[1000px]">
                  ...
                  <tbody>
                    {filteredList.map((nfe) => {
                      const statusInfo = NFE_STATUS_LABELS[nfe.status] || {
                        label: nfe.status,
                      ...
                      return (
                        <tr key={nfe.id} className="border-b hover:bg-muted/50">
                          <td className="p-4 font-medium">
                            {nfe.serie}/{nfe.numero}
                          </td>
                          <td className="p-4 font-mono text-xs">
                            {nfe.access_key
                              ? nfe.access_key.slice(0, 10) +
                                "..." +
                                nfe.access_key.slice(-10)
                              : "-"}
                          </td>
                          ...
                          <td className="p-4 text-sm text-muted-foreground">
                            {new Date(nfe.created_at).toLocaleDateString(
                              "pt-BR",
                            )}
                          </td>
                            ...
                            <div className="flex justify-end gap-1">
                              {nfe.status === "gerada" && (
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => handleSign(nfe)}
                                  disabled={actionLoading === `sign-${nfe.id}`}
                                  title="Assinar com A1"
                                >
                                  {actionLoading === `sign-${nfe.id}` ? (
                              ...
                              {(nfe.status === "gerada" ||
                                ...
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => handleDeleteNFe(nfe.id)}
                                  disabled={
                                    actionLoading === `delete-${nfe.id}`
                                  }
                                  title="Excluir/Descartar Nota Local"
                              ...
                              {nfe.status === "assinada" && (
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => handleSendToSEFAZ(nfe)}
                                  disabled={actionLoading === `send-${nfe.id}`}
                                  title="Enviar para SEFAZ"
                                >
                                  {actionLoading === `send-${nfe.id}` ? (
                                    <RefreshCw className="h-4 w-4 animate-spin" />
                                  ) : (
      ...
      <Dialog open={configOpen} onOpenChange={setConfigOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Configurações da NF-e (Emitente)</DialogTitle>
            <DialogDescription>
              Configure os dados da empresa emitente para emissão de NF-e
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setConfig({ ...config, razao_social: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setConfig({ ...config, nome_fantasia: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setConfig({ ...config, cnpj: formatted });
                      }}
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={config.crt || "3"}
                    onValueChange={(v) => setConfig({ ...config, crt: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      setConfig({ ...config, telefone: formatted });
                    }}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setConfig({ ...config, serie: Number(e.target.value) })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        proximo_numero: Number(e.target.value),
                      })
                    }
                  />
                </div>
            ...
            <TabsContent value="endereco" className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    value={config.endereco?.codigo_municipio || ""}
                    onChange={(e) =>
                      setConfig({
                        ...config,
                        endereco: {
                ...
                <div className="space-y-2">
                  <Label>UF</Label>
                  <Select
                    value={config.endereco?.uf || "SP"}
                    onValueChange={(v) =>
                      setConfig({
                        ...config,
                        endereco: { ...config.endereco!, uf: v },
                      })
                    }
                  >
                    <SelectTrigger>
                      ...
                      onChange={(e) => {
                        ...
                        setConfig({
                          ...
                          endereco: {
                            ...
                            municipio: config.endereco?.municipio || "",
                            codigo_municipio:
                              config.endereco?.codigo_municipio || "",
                            uf: config.endereco?.uf || "SP",
                            cep: formatted,
                          },
                        });
                      }}
              ...
              <div className="space-y-4">
                <div className="bg-muted/50 p-4 rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground mb-4">
                    Faca upload do seu certificado digital no formato .pfx ou
                    .p12
                  </p>
                  <div className="space-y-4">
                    <div className="space-y-2">
                ...
                {certInfo && (
                  <div className="bg-green-50 border border-green-200 p-4 rounded-lg">
                    ...
                    <div className="text-sm text-green-700 space-y-1">
                      ...
                      <p>
                        <strong>Emissor:</strong> {certInfo.issuer}
                      </p>
                      <p>
                        <strong>Valido ate:</strong>{" "}
                        {new Date(certInfo.validTo).toLocaleDateString("pt-BR")}
                      </p>
                      {certInfo.daysUntilExpiry < 30 && (
                        <p className="text-red-600 font-medium">
                          Atenção: Certificado expira em{" "}
      ...
      >
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Selecione um pedido e revise os dados antes de gerar a nota fiscal
            </DialogDescription>
          </DialogHeader>
                  ...
                  <ScrollArea className="h-48">
                    {orders.map((order) => (
                      <SelectItem key={order.id} value={order.id}>
                        #{order.id.slice(0, 8)} - {order.customer_name} - R${" "}
                        {Number(order.total_amount || order.total || 0).toFixed(
                          2,
                        )}
                      </SelectItem>
                    ))}
            ...
            {!preparingDraft && draftDestinatario && (
              ...
              >
                <TabsList className="grid w-full grid-cols-3 mb-4">
                  ...
                  <TabsTrigger value="endereco_dest">
                    Endereço de Entrega
                  </TabsTrigger>
                  <TabsTrigger value="itens_dest">
                    Produtos / Itens ({draftItens.length})
                  </TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="cliente" className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2 col-span-2">
                      ...
                      <Input
                        value={draftDestinatario.nome || ""}
                        onChange={(e) =>
                          setDraftDestinatario({
                            ...draftDestinatario,
                            nome: e.target.value,
                          })
                        }
                    ...
                    <div className="space-y-2">
                      <Label>Inscrição Estadual</Label>
                      <Input
                        value={draftDestinatario.inscricaoEstadual || ""}
                        onChange={(e) =>
                          setDraftDestinatario({
                            ...draftDestinatario,
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        value={draftDestinatario.indIEDest || "9"}
                        onValueChange={(v) =>
                          setDraftDestinatario({
                            ...draftDestinatario,
                            indIEDest: v,
                ...
                <TabsContent value="endereco_dest" className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2 col-span-2">
                      <Label>Logradouro *</Label>
                      <Input
                        value={draftDestinatario.endereco?.logradouro || ""}
                        onChange={(e) =>
                          setDraftDestinatario({
                            ...draftDestinatario,
                            endereco: {
                              ...draftDestinatario.endereco,
                              logradouro: e.target.value,
                            },
                          })
                        }
                    ...
                    <div className="space-y-2">
                      <Label>Número *</Label>
                      <Input
                        value={draftDestinatario.endereco?.numero || ""}
                        onChange={(e) =>
                          setDraftDestinatario({
                            ...draftDestinatario,
                            endereco: {
                    ...
                    <div className="space-y-2">
                      <Label>Complemento</Label>
                      <Input
                        value={draftDestinatario.endereco?.complemento || ""}
                        onChange={(e) =>
                          setDraftDestinatario({
                            ...draftDestinatario,
                            endereco: {
                    ...
                    <div className="space-y-2">
                      <Label>Bairro *</Label>
                      <Input
                        value={draftDestinatario.endereco?.bairro || ""}
                        onChange={(e) =>
                          setDraftDestinatario({
                            ...draftDestinatario,
                            endereco: {
                    ...
                    <div className="space-y-2">
                      <Label>CEP *</Label>
                      <Input
                        value={draftDestinatario.endereco?.cep || ""}
                        onChange={(e) =>
                          setDraftDestinatario({
                            ...draftDestinatario,
                            endereco: {
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        value={draftDestinatario.endereco?.municipio || ""}
                        onChange={(e) =>
                          setDraftDestinatario({
                            ...draftDestinatario,
                            endereco: {
                    ...
                    <div className="space-y-2">
                      <Label>UF *</Label>
                      <Select
                        value={draftDestinatario.endereco?.uf || "SP"}
                        onValueChange={(v) =>
                          setDraftDestinatario({
                            ...draftDestinatario,
                            endereco: { ...draftDestinatario.endereco, uf: v },
                          })
                        }
                      >
                        <SelectTrigger>
                ...
                <TabsContent value="itens_dest" className="space-y-4">
                  <ScrollArea className="h-72 border rounded-md p-2">
                    {draftItens.map((item, idx) => (
                      <div
                        key={idx}
                        className="p-4 border border-slate-100 rounded-md mb-2 space-y-3 bg-muted/10"
                      >
                        <div className="font-semibold text-sm flex justify-between items-center bg-muted/40 p-2 rounded">
                          <span>
                            Item #{idx + 1} - {item.descricao}
                          </span>
                          ...
                          <div className="space-y-1">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                newItens[idx].valorTotal =
                                  qty * newItens[idx].valorUnitario;
                                newItens[idx].valorBaseCalculo =
                                  qty * newItens[idx].valorUnitario;
                                newItens[idx].valorICMS =
                                  qty *
                                  newItens[idx].valorUnitario *
                                  (Number(newItens[idx].aliquotaICMS || 18) /
                                    100);
                                setDraftItens(newItens);
                              }}
                            />
      ...
      <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              NF-e: Serie {selectedNFe?.serie} / Nº {selectedNFe?.numero}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label>Motivo do Cancelamento * (min. 15 caracteres)</Label>
              <textarea
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              disabled={actionLoading === "cancel" || cancelReason.length < 15}
            >
      ...
      <Dialog open={signerInfoOpen} onOpenChange={setSignerInfoOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Informacoes sobre o uso do certificado
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="bg-muted p-4 rounded-lg space-y-2 text-sm">
              ...
              <p className="text-muted-foreground">
                O certificado A1 (.pfx) e suportado diretamente pelo servidor.
                Faca o upload na aba "Certificado Digital" nas configuracoes.
              </p>
            </div>
            {signerInfo?.info && (
              <div className="space-y-2 text-sm">
                <h4 className="font-semibold">Informacoes:</h4>
                <div className="bg-muted p-3 rounded font-mono text-xs space-y-1">
                  <p>
                    <strong>Descricao:</strong> {signerInfo.info.description}
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="flex justify-end">
            <Button variant="outline" onClick={() => setSignerInfoOpen(false)}>
              Fechar
            </Button>
          </div>
        </DialogContent>
